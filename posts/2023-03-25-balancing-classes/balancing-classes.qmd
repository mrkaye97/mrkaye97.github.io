...
In [my last post](https://matthewrkaye.com/posts/2023-03-20-on-class-imbalances-in-ml/calibration.html) I wrote about common classifications metrics and, especially, calibration.
...
For the purposes of this example, I'll use the Wisconsin breast cancer data. The data is build into the `mlbench` package in R and `scikit-learn` in python. You can also get it from the [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Wisconsin+(Diagnostic)).
...
Now, let's compare the intercept coefficients of these two models on a probability (read: not a log-odds) scale.
...
In the [previous post](https://matthewrkaye.com/posts/2023-03-20-on-class-imbalances-in-ml/calibration.html), I wrote about calibration. In short, I think calibration is the single most important metric in evaluating the performance of classification models.
