...
These incidents were all preventable. Namely, typed, compiled languages would prevent these types of issues for free - if you made any of these mistakes, at least in most cases, your code would fail to compile and would never make it to production. For example, in TypeScript, the first example above would throw an error like `Argument of type '{ ba: string; }' is not assignable to parameter of type '{ bar: string; }'` at compile time. In the second case, an ORM or a tool like [sqlc](https://sqlc.dev/) would ensure type parity between the database and the code (this has an asterisk next to it, since there are some possible footguns here). And finally, `model_validate` [can be called on an object of any type](https://docs.pydantic.dev/latest/api/base_model/#pydantic.BaseModel.model_rebuild), meaning that you could pass a dictionary or other object missing some required fields into that function and your code (before runtime) would assume it would correctly parse to whatever your model is defined as, and would throw a validation error if that's not possible.

On a lot of Python (and likely JS, Ruby, etc.) teams, these types of errors are viewed as the price you pay for being able to move quickly, and the other benefits and flexibility that you get from a dynamically typed, interpreted language like Python. That doesn't need to be the case! At Klaviyo and ZenSearch, we make heavy use of [MyPy](https://mypy.readthedocs.io/en/stable/index.html), which is a static type checker for Python, to help us solve some of these problems.
...
For the second issue, a possible fix might be to have a linter that checks that the models in your (e.g.) `models.py` are in sync with the state of the database, and run that linter in CI. In this case, that check would have failed, since the models had `nullable=False` while the database allowed the column to be nullable.

For the third, there are two paths:

1. Manually type out all of the fields instead of using `model_validate`, which will allow the types to be checked by MyPy.
2. Have tests of code that uses `model_validate` so you'll know if something ha been broken.

## Rules of Thumb

In general, there are a couple rules of thumb I try to follow when working in Python that help me write code that's easier to maintain over time:

1. Typed / structured objects everywhere. It doesn't need to be Pydantic, but using classes (`dataclasses` or otherwise) to represent data being passed around your application code as opposed to using dictionaries, tuples, or similar is a great way to help the type checker figure out what's what.
2. Writing tests to cover rough edges, such as validation of incoming data from a database, a webhook, etc. can help you be confident that your code will do what you want.
3. Limiting the use of escape hatches like `Any` and `# type: ignore`, which essentially prevent any type checking from happening are a good idea. While these can be convenient in the short term, I generally try to limit use of typing escape hatches as much as possible (or not use them at all) in order to have as much of my code as possible be typed, and, therefore, type checked.
4. The [typing](https://docs.python.org/3/library/typing.html) module in Python provides some great tooling for stricter and more specific typing, such as generics, `ParamSpec` for decorating functions like decorators, `Protocols`, and more! In general, my view has been that the more specific the types, the fewer the possible error cases.
